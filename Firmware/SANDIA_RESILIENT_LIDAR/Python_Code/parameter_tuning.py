...
# Diagnostics recorder options.
DIAG_ENABLE = True
DIAG_OUTPUT_ROOT = os.path.join("captures", "diag_runs")
DIAG_RUN_LABEL = "static_indoor_downstairs"
DIAG_RUN_NOTE = ""
DIAG_LOG_EVERY_N_FRAMES = 1
DIAG_SAVE_DEBUG_NPZ_EVERY = 30
DIAG_SAVE_NPZ_ON_TRIGGER_EDGE = True
DIAG_START_AFTER_MANUAL_B = True

# Intruder algorithm tuning.
DIFF_THRESHOLD_M = 0.1
MIN_BLOB_AREA_PIXELS = 25
BLOB_TRIGGER_PIXELS = 30
TEMPORAL_TRIGGER_FRAMES = 3
MASK_BLUR_KERNEL = 11
...
AUTO_BACKGROUND_UPDATE_FRAMES = 100
...
DIAG_FRAME_COLUMNS = [
	...
	"foreground_pixels",
	"largest_blob_pixels",
	"smoothed_blob_pixels",
	"trigger_metric_pixels",
	"depth_candidate_pixels",
	"sat_transition_pixels",
	"validity_transition_pixels",
	"low_confidence_reject_pixels",
	"post_blur_pixels",
	"small_blob_reject_pixels",
	"amp_conf_mean",
	"amp_conf_p10",
	"amp_conf_p90",
	"diff_mean_m",
	...
	"invalid_ratio",
	"DIFF_THRESHOLD_M",
	"MIN_BLOB_AREA_PIXELS",
	"BLOB_TRIGGER_PIXELS",
	"TEMPORAL_TRIGGER_FRAMES",
	"MASK_BLUR_KERNEL",
	"MIN_DETECTION_CONFIDENCE",
	"LOW_CONF_THRESHOLD_SCALE",
	"ENABLE_ADAPTIVE_DIFF_THRESHOLD",
	"ENABLE_SATURATION_TRANSITION_TRIGGER",
	"ENABLE_VALIDITY_TRANSITION_TRIGGER",
	"run_label",
]


def _sanitize_run_label(label):
	text = (label or "run").strip()
...
def _array_stat_or_blank(array, fn):
	if array is None:
		return ""
...
def get_runtime_config_snapshot():
	return {
		...
		"MIN_BLOB_AREA_PIXELS": MIN_BLOB_AREA_PIXELS,
		"BLOB_TRIGGER_PIXELS": BLOB_TRIGGER_PIXELS,
		"TEMPORAL_TRIGGER_FRAMES": TEMPORAL_TRIGGER_FRAMES,
		"MASK_BLUR_KERNEL": MASK_BLUR_KERNEL,
		"ENABLE_MASK_BLUR": ENABLE_MASK_BLUR,
		"ENABLE_DEPTH_PRE_FILTER": ENABLE_DEPTH_PRE_FILTER,
		"DEPTH_FILTER_KERNEL": DEPTH_FILTER_KERNEL,
		"ENABLE_ADAPTIVE_DIFF_THRESHOLD": ENABLE_ADAPTIVE_DIFF_THRESHOLD,
		"ADAPTIVE_AMP_LOW_LSB": ADAPTIVE_AMP_LOW_LSB,
		"ADAPTIVE_AMP_HIGH_LSB": ADAPTIVE_AMP_HIGH_LSB,
		"LOW_CONF_THRESHOLD_SCALE": LOW_CONF_THRESHOLD_SCALE,
		"MIN_DETECTION_CONFIDENCE": MIN_DETECTION_CONFIDENCE,
		"ENABLE_VALIDITY_TRANSITION_TRIGGER": ENABLE_VALIDITY_TRANSITION_TRIGGER,
		"VALIDITY_TRANSITION_MIN_CONFIDENCE": VALIDITY_TRANSITION_MIN_CONFIDENCE,
		"ENABLE_SATURATION_TRANSITION_TRIGGER": ENABLE_SATURATION_TRANSITION_TRIGGER,
		"AUTO_BACKGROUND_UPDATE_FRAMES": AUTO_BACKGROUND_UPDATE_FRAMES,
		"INVALIDATE_ZERO_CLAMP_PIXELS": INVALIDATE_ZERO_CLAMP_PIXELS,
		"ZERO_DEPTH_INVALID_EPS_M": ZERO_DEPTH_INVALID_EPS_M,
		"ENABLE_KALMAN": ENABLE_KALMAN,
		"KALMAN_USE_FOR_TRIGGER": KALMAN_USE_FOR_TRIGGER,
		"KALMAN_MAX_MISSED_FRAMES": KALMAN_MAX_MISSED_FRAMES,
		"KALMAN_PROCESS_NOISE": KALMAN_PROCESS_NOISE,
		"KALMAN_MEASUREMENT_NOISE": KALMAN_MEASUREMENT_NOISE,
	}


def open_diagnostics_run(state):
	if not DIAG_ENABLE:
		return

	run_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
	run_name = f"run_{run_stamp}_{_sanitize_run_label(DIAG_RUN_LABEL)}"
	run_dir = os.path.join(DIAG_OUTPUT_ROOT, run_name)
	os.makedirs(run_dir, exist_ok=True)

	frames_csv_path = os.path.join(run_dir, "frames.csv")
	markers_csv_path = os.path.join(run_dir, "markers.csv")
	debug_npz_dir = os.path.join(run_dir, "debug_npz")
	os.makedirs(debug_npz_dir, exist_ok=True)

	manifest = {
		"schema_version": DIAG_SCHEMA_VERSION,
		"created_at": datetime.now().isoformat(),
		"run_label": DIAG_RUN_LABEL,
		"run_note": DIAG_RUN_NOTE,
	...
	frames_file = open(frames_csv_path, "w", newline="", encoding="utf-8")
	frames_writer = csv.DictWriter(frames_file, fieldnames=DIAG_FRAME_COLUMNS)
	frames_writer.writeheader()

	markers_file = open(markers_csv_path, "w", newline="", encoding="utf-8")
	markers_writer = csv.DictWriter(markers_file, fieldnames=["wall_time", "frame_idx", "event", "note"])
	markers_writer.writeheader()

	state["diag_run_dir"] = run_dir
	state["diag_debug_npz_dir"] = debug_npz_dir
	state["diag_frames_file"] = frames_file
	state["diag_frames_writer"] = frames_writer
	state["diag_markers_file"] = markers_file
	state["diag_markers_writer"] = markers_writer

	log_event(f"Diagnostics run opened: {run_dir}")


def close_diagnostics_run(state):
	for key in ("diag_frames_file", "diag_markers_file"):
		fh = state.get(key)
		if fh is not None:
			try:
				fh.close()
...
def log_marker_event(state, event_name, note=""):
	if not DIAG_ENABLE:
		return
	if not state.get("diag_logging_enabled", True):
		return
	writer = state.get("diag_markers_writer")
	fh = state.get("diag_markers_file")
	if writer is None or fh is None:
		return
	writer.writerow(
		{
			"wall_time": timestamp_text(),
	...
	fh.flush()
	state["diag_marker_count"] = int(state.get("diag_marker_count", 0)) + 1
	log_event(f"Marker: {event_name}")


def log_diagnostics_frame(state, sat_mask, low_signal_mask, zero_clamped_mask):
	if not DIAG_ENABLE:
		return
	if not state.get("diag_logging_enabled", True):
		return
	writer = state.get("diag_frames_writer")
	fh = state.get("diag_frames_file")
	if writer is None or fh is None:
		return

	total_pixels = float(FRAME_WIDTH * FRAME_HEIGHT)
	diff_m = state.get("diff_m")
	adaptive_thr = state.get("adaptive_threshold_m")
	amp_conf = state.get("amp_confidence")

	row = {
		"wall_time": timestamp_text(),
		"unix_time_s": time.time(),
		"frame_idx": int(state.get("frame_index", 0)),
		"paused": int(bool(state.get("paused", False))),
		"armed": int(state.get("ref_depth_m") is not None),
		"triggered": int(bool(state.get("triggered", False))),
		"trigger_counter": int(state.get("trigger_counter", 0)),
		"bg_update_applied": int(bool(state.get("diag_bg_update_applied", False))),
		"modulation_hz": float(state.get("modulation_hz", 0.0)),
		"max_depth_m": float(state.get("max_depth_m", 0.0)),
		"integration_slot": "" if state.get("integration_slot") is None else int(state.get("integration_slot")),
		"integration_raw": "" if state.get("integration_raw") is None else int(state.get("integration_raw")),
		"foreground_pixels": int(state.get("foreground_pixels", 0)),
		"largest_blob_pixels": int(state.get("largest_blob_pixels", 0)),
		"smoothed_blob_pixels": _finite_or_blank(state.get("smoothed_blob_pixels", "")),
		"trigger_metric_pixels": _finite_or_blank(state.get("trigger_metric_pixels", "")),
		"depth_candidate_pixels": int(state.get("diag_depth_candidate_pixels", 0)),
		"sat_transition_pixels": int(state.get("diag_sat_transition_pixels", 0)),
		"validity_transition_pixels": int(state.get("diag_validity_transition_pixels", 0)),
		"low_confidence_reject_pixels": int(state.get("diag_low_confidence_reject_pixels", 0)),
		"post_blur_pixels": int(state.get("diag_post_blur_pixels", 0)),
		"small_blob_reject_pixels": int(state.get("diag_small_blob_reject_pixels", 0)),
		"amp_conf_mean": _array_stat_or_blank(amp_conf, np.mean),
		"amp_conf_p10": _array_stat_or_blank(amp_conf, lambda a: np.percentile(a, 10.0)),
		"amp_conf_p90": _array_stat_or_blank(amp_conf, lambda a: np.percentile(a, 90.0)),
		"diff_mean_m": _array_stat_or_blank(diff_m, np.mean),
		"diff_p95_m": _array_stat_or_blank(diff_m, lambda a: np.percentile(a, 95.0)),
		"diff_p99_m": _array_stat_or_blank(diff_m, lambda a: np.percentile(a, 99.0)),
		"diff_p999_m": _array_stat_or_blank(diff_m, lambda a: np.percentile(a, 99.9)),
		"adaptive_thr_mean_m": _array_stat_or_blank(adaptive_thr, np.mean),
		"saturated_ratio": float(np.count_nonzero(sat_mask) / total_pixels),
		"low_signal_ratio": float(np.count_nonzero(low_signal_mask) / total_pixels),
		"zero_clamped_ratio": float(np.count_nonzero(zero_clamped_mask) / total_pixels),
		"invalid_ratio": float(np.count_nonzero(state.get("display_invalid_mask", np.zeros((FRAME_HEIGHT, FRAME_WIDTH), dtype=bool))) / total_pixels),
		"DIFF_THRESHOLD_M": DIFF_THRESHOLD_M,
		"MIN_BLOB_AREA_PIXELS": MIN_BLOB_AREA_PIXELS,
		"BLOB_TRIGGER_PIXELS": BLOB_TRIGGER_PIXELS,
		"TEMPORAL_TRIGGER_FRAMES": TEMPORAL_TRIGGER_FRAMES,
		...
		"MIN_DETECTION_CONFIDENCE": MIN_DETECTION_CONFIDENCE,
		"LOW_CONF_THRESHOLD_SCALE": LOW_CONF_THRESHOLD_SCALE,
		"ENABLE_ADAPTIVE_DIFF_THRESHOLD": int(ENABLE_ADAPTIVE_DIFF_THRESHOLD),
		"ENABLE_SATURATION_TRANSITION_TRIGGER": int(ENABLE_SATURATION_TRANSITION_TRIGGER),
		"ENABLE_VALIDITY_TRANSITION_TRIGGER": int(ENABLE_VALIDITY_TRANSITION_TRIGGER),
		"run_label": DIAG_RUN_LABEL,
	}
...
def maybe_save_diagnostic_npz(state, reason, sat_mask, low_signal_mask, zero_clamped_mask, amplitude_lsb):
	if not DIAG_ENABLE:
		return
	if not state.get("diag_logging_enabled", True):
		return
	dir_path = state.get("diag_debug_npz_dir")
	if not dir_path:
		return

	frame_idx = int(state.get("frame_index", 0))
	path = os.path.join(dir_path, f"diag_{frame_idx:07d}_{reason}.npz")
	np.savez_compressed(
		path,
		reason=reason,
		frame_idx=frame_idx,
		depth_m=state.get("depth_m"),
		depth_filtered_m=state.get("depth_filtered_m"),
		diff_m=state.get("diff_m"),
		adaptive_threshold_m=state.get("adaptive_threshold_m"),
		amp_confidence=state.get("amp_confidence"),
		amplitude_lsb=amplitude_lsb,
		candidate_mask=state.get("candidate_mask"),
		post_blur_mask=state.get("post_blur_mask"),
		fg_mask=state.get("fg_mask"),
		sat_mask=sat_mask,
		low_signal_mask=low_signal_mask,
		zero_clamped_mask=zero_clamped_mask,
		ref_depth_m=state.get("ref_depth_m"),
	)
...
def update_intruder_detection(state, depth_m, sat_mask, amplitude_lsb, extra_invalid_mask=None):
	...
	state["diag_depth_candidate_pixels"] = 0
	state["diag_sat_transition_pixels"] = 0
	state["diag_validity_transition_pixels"] = 0
	state["diag_low_confidence_reject_pixels"] = 0
	state["diag_post_blur_pixels"] = 0
	state["diag_small_blob_reject_pixels"] = 0
	...
	if state["ref_depth_m"] is not None:
		...
		# Baseline foreground from depth delta before confidence gating.
		bg_valid_mask = np.isfinite(ref)
		depth_candidate = valid_mask & bg_valid_mask & (diff >= adaptive_threshold_m)
		low_conf_reject = depth_candidate & (~confidence_mask)
		candidate = depth_candidate & confidence_mask

		sat_transition = np.zeros(depth_for_model.shape, dtype=bool)
		validity_change = np.zeros(depth_for_model.shape, dtype=bool)
		...
		state["diag_depth_candidate_pixels"] = int(np.count_nonzero(depth_candidate))
		state["diag_sat_transition_pixels"] = int(np.count_nonzero(sat_transition))
		state["diag_validity_transition_pixels"] = int(np.count_nonzero(validity_change))
		state["diag_low_confidence_reject_pixels"] = int(np.count_nonzero(low_conf_reject))
		...
		state["diag_post_blur_pixels"] = int(np.count_nonzero(post_blur_mask))
		...
		for label_idx in range(1, num_labels):
			...
			if blob_area >= MIN_BLOB_AREA_PIXELS:
				...
				small_blob_reject_pixels += blob_area
		state["diag_small_blob_reject_pixels"] = small_blob_reject_pixels
...
def capture_background(state):
	...
	if DIAG_ENABLE and DIAG_START_AFTER_MANUAL_B and (not state.get("diag_logging_enabled", False)):
		state["diag_logging_enabled"] = True
		log_event("Diagnostics logging enabled by manual background capture")
	log_event("Background captured (filtered depth reference)")


def maybe_auto_update_background(state):
	state["diag_bg_update_applied"] = False
	...
	if state["ref_depth_m"] is None:
		state["ref_depth_m"] = state["depth_filtered_m"].copy()
		state["frames_since_bg_update"] = 0
		state["diag_bg_update_applied"] = True
...
def build_workflow_view(depth_bgr, state):
	...
	if state["depth_filtered_m"] is not None:
		prefilter_invalid = ~np.isfinite(state["depth_filtered_m"])
		prefilter_bgr = depth_to_colormap(state["depth_filtered_m"], prefilter_invalid, state["max_depth_m"])
	else:
		prefilter_bgr = np.zeros((FRAME_HEIGHT, FRAME_WIDTH, 3), dtype=np.uint8)
	prefilter_panel = label_panel(prefilter_bgr, "Depth (Pre-Filter)")
	...
	row1 = np.hstack((depth_panel, prefilter_panel, diff_panel))
	row2 = np.hstack((candidate_panel, blob_panel, overlay_panel))
...
def draw_overlay(display_bgr, state):
	...
	draw_outlined_text(
		view,
		"markers: 1 intruder_start | 2 intruder_end | n nuisance | x artifact | m mark+save",
		(10, 72),
		font_scale=0.46,
		thickness=1,
	)
	...
	draw_outlined_text(view, int_text, (10, 90), font_scale=0.50, thickness=1)
	...
	if DIAG_ENABLE:
		draw_outlined_text(
			view,
			f"diag frame={state.get('frame_index', 0)} markers={state.get('diag_marker_count', 0)} label={DIAG_RUN_LABEL}",
			(10, 210),
			font_scale=0.48,
			thickness=1,
		)
	...
	if state["clicked_uv"] is not None and state["depth_m"] is not None:
		...
		draw_outlined_text(view, click_text, (10, 250), font_scale=0.52, thickness=1)
...
def save_snapshot(state):
	...
	if state["display_bgr"] is not None:
		overlay = draw_overlay(state["display_bgr"], state) if state.get("show_overlay", True) else state["display_bgr"]
		cv2.imwrite(png_path, overlay)
...
def main():
	...
	state = {
		...
		"frame_index": 0,
		"diag_marker_count": 0,
		"diag_bg_update_applied": False,
		"diag_depth_candidate_pixels": 0,
		"diag_sat_transition_pixels": 0,
		"diag_validity_transition_pixels": 0,
		"diag_low_confidence_reject_pixels": 0,
		"diag_post_blur_pixels": 0,
		"diag_small_blob_reject_pixels": 0,
		"diag_run_dir": None,
		"diag_debug_npz_dir": None,
		"diag_frames_file": None,
		"diag_frames_writer": None,
		"diag_markers_file": None,
		"diag_markers_writer": None,
		"diag_logging_enabled": not DIAG_START_AFTER_MANUAL_B,
	}
	cv2.setMouseCallback(WINDOW_NAME, on_mouse, state)
	open_diagnostics_run(state)
	if DIAG_ENABLE and DIAG_START_AFTER_MANUAL_B:
		log_event("Diagnostics waiting for manual background capture (press 'b')")
		...
		while True:
			...
			try:
				...
				if not state["paused"]:
					...
					if (
						DIAG_ENABLE
						and state.get("diag_logging_enabled", True)
						and (state["frame_index"] % max(1, DIAG_LOG_EVERY_N_FRAMES) == 0)
					):
						log_diagnostics_frame(state, sat_mask, low_signal_mask, zero_clamped_mask)

					if (
						DIAG_ENABLE
						and state.get("diag_logging_enabled", True)
						and DIAG_SAVE_DEBUG_NPZ_EVERY > 0
						and (state["frame_index"] % DIAG_SAVE_DEBUG_NPZ_EVERY) == 0
					):
						maybe_save_diagnostic_npz(
							state,
							reason="periodic",
							sat_mask=sat_mask,
							low_signal_mask=low_signal_mask,
					...
					if (
						DIAG_ENABLE
						and state.get("diag_logging_enabled", True)
						and DIAG_SAVE_NPZ_ON_TRIGGER_EDGE
						and (not was_triggered)
						and state["triggered"]
					):
						maybe_save_diagnostic_npz(
							state,
					...
					if (not DIAG_ENABLE) or state.get("diag_logging_enabled", True):
				...
				if state["display_bgr"] is not None:
					view = draw_overlay(state["display_bgr"], state) if state["show_overlay"] else state["display_bgr"]
				...
				if key == ord("s"):
					save_snapshot(state)
					log_marker_event(state, "manual_snapshot")
				if key == ord("b"):
					capture_background(state)
					log_marker_event(state, "background_capture")
				if key == ord("r"):
					rearm_detection(state)
					log_marker_event(state, "rearm")
				if key == ord("w"):
				...
				if key == ord("1"):
					log_marker_event(state, "intrusion_start")
				if key == ord("2"):
					log_marker_event(state, "intrusion_end")
				if key == ord("n"):
					log_marker_event(state, "nuisance_event")
				if key == ord("x"):
					log_marker_event(state, "artifact_event")
				if key == ord("m"):
					log_marker_event(state, "manual_mark")
					save_snapshot(state)
				if key == ord("i"):
